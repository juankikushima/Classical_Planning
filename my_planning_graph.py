...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        #compare every effect of actionA and check if it negates effects from actionB
        for effectA in actionA.effects:
            if ~effectA in actionB.effects:
                return True
        #compare every effect of actionB and check if it negates effects from actionA
        #...seems to be unnecessary, redundant
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        #check negation for every effect of actionA to every precondition of actionB
        for effect in actionA.effects:
            if ~effect in actionB.preconditions:
                return True
        #check negation for every effect of actionB to every precondition of actionA
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for precondsA in actionA.preconditions:
            for precondsB in actionB.preconditions:
                if self.parent_layer.is_mutex(precondsA, precondsB):
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        for precondsA in self.parents[literalA]:
            for precondsB in self.parents[literalB]:
                if self.parent_layer.is_mutex(precondsA, precondsB):
                    continue
                else:
                    return False
    ...
    def _negation(self, literalA, literalB):
        ...
        # TODO: implement this function
        #original: return literalA == ~literalB and literalB == ~literalA ... seems redundant
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        """
        # TODO: implement this function
        #start with levelsum value at 0, add each goal's level where it first appears
        levelSum = 0
        goalLevel = 0 #increases as level extends
        ...
        while len(goals) != 0 :
            ...
            for goal in goals:
                if goal in self.literal_layers[goalLevel]:
    ...
    def h_maxlevel(self):
        ...
        while len(goals) != 0:
            ...
            for goal in goals:
                if goal in self.literal_layers[maxLevel]:
                    goalsCopy.remove(goal)
    ...
    def h_setlevel(self):
        ...
        def noMutexPairs():
            goalPairs = combinations(goals, 2)
            for pair in goalPairs:
                if self.literal_layers[setLevel].is_mutex(pair[0],pair[1]):
                    return False
            return True

        while not setLevelFound:
            allGoalsFound = all(goal in self.literal_layers[setLevel] for goal in goals)
            if allGoalsFound and noMutexPairs():
                setLevelFound = True
            else:
                self._extend()
                setLevel += 1
